...
int ISSConverter::ConvertFile( std::string input_file_name,
	...
	// Read the file. // need to transfer this file to spy and get it
	...
	for( unsigned long nblock = 0; nblock < BLOCKS_NUM ; nblock++ ){
        ...
        if (GetFLagSpy()){
            x_header = dataSpyClass.dataSpyRead(nblock, (char*)&block_header,  HEADER_SIZE);
            x_data = dataSpyClass.dataSpyRead(nblock, (char*)&block_data,   MAIN_SIZE);
        }else{
            x_header =  nblock;
            x_data =  nblock;
        }
        ...
        ProcessBlockHeader(x_header); // for spy data
		// Process the main block data until terminator found
		data = (ULong64_t *)(x_header);
		//ProcessBlockData( nblock );
        ProcessBlockData(x_data);  // for spy data
